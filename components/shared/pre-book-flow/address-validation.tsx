...
export function AddressValidation({
  ...
  const fetchSuggestions = useCallback(async () => {
    if (!value || value.length < 2 || !autocompleteService || !sessionToken) return;
  ...
  }, [value, autocompleteService, sessionToken, bounds, fetchPostcodeSuggestions]);

  useEffect(() => {
    if (!value || value.length < 2 || !isInputFocused) {
      setSuggestions([]);
      return;
    }
